...
- [ ] `Todo` 타입에 `id`, `text`, `completed` 필드를 정의한다.
- [ ] 입력 컴포넌트에서 비어 있지 않은 Todo를 추가한다.
- [ ] 목록 컴포넌트에서 Todo를 표시한다.
- [ ] 항목 컴포넌트에서 완료 상태를 변경하고 삭제한다.
- [ ] 상태 관리는 `App.tsx`의 React 상태로 통일한다.
- [ ] 기본 기능을 벗어나는 필터, 서버 저장, 인증은 추가하지 않는다.
...
- [ ] Todo 추가, 완료 상태 변경, 삭제 동작을 테스트한다.
- [ ] `npm test`를 실행해 테스트가 통과하는지 확인한다.
- [ ] `npm run lint`를 실행해 정적 검사가 통과하는지 확인한다.
- [ ] `npm run build`를 실행해 배포 빌드가 성공하는지 확인한다.
- [ ] 실패하면 원인을 수정하고 세 명령을 다시 실행한다.
...
- [ ] 현재 구현과 상태 관리 방식을 먼저 탐색한다.
- [ ] 완료 항목 표시 여부를 관리할 최소 상태 또는 필터 방식을 계획한다.
- [ ] 계획에 없는 파일은 수정하지 않는다.
- [ ] 완료 항목 숨기기 UI와 필터 동작을 구현한다.
- [ ] 완료 항목 숨김 상태와 해제 상태를 테스트한다.
...
- [ ] 프로젝트 지침의 test, lint, build 명령을 순서대로 실행한다.
- [ ] 실패한 명령과 원인을 확인한다.
- [ ] 실패 원인에 해당하는 구현 또는 테스트 파일만 수정한다.
...
- [ ] 요구사항이 모두 구현되었는지 확인한다.
- [ ] 불필요한 파일과 변경이 없는지 확인한다.
- [ ] 테스트가 실제 동작을 검증하는지 확인한다.
- [ ] 프로젝트 지침을 어긴 구현이 없는지 확인한다.
- [ ] `git status`와 `git diff`를 확인한다.
- [ ] 사람이 최종 변경 내용을 검토하고 완료 여부를 결정한다.
